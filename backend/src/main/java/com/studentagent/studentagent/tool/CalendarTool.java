...
public class CalendarTool {
     ...
     * 查询用户在指定时间范围内的事件
     */
    @Tool(description = "查询用户在指定日期范围内的日历事件。用于回答「我下周有什么安排」「8月有什么计划」等问题")
     ...
     * 添加日历事件
     */
    @Tool(description = "在用户日历中添加一个学习事件/任务。用于「帮我在8月5号安排复习高数」「下周一加上背单词任务」等")
    public String addEvent(
            @ToolParam(description = "事件标题，如「复习高等数学」「做线性代数题」") String title,
            @ToolParam(description = "开始日期，格式 yyyy-MM-dd，如 2026-08-05") String startDate,
            @ToolParam(description = "结束日期（可选），格式 yyyy-MM-dd。如果是单日事件则与开始日期相同") String endDate,
            @ToolParam(description = "事件类型：plan(学习计划)、task(任务)、exam(考试)、review(复习/艾宾浩斯复习)，默认 task") String eventType,
            ToolContext toolContext) {
     ...
     */
    @Tool(description = "删除用户日历中的事件。用于「把基础阶段删掉」「把8月5号的高数删掉」「取消所有强化阶段」等。" +
            "title会做模糊匹配，date为空字符串时删除所有匹配标题的事件（跨日期范围也适用）")
    public String deleteEvent(
            @ToolParam(description = "要删除的事件标题（模糊匹配），如「基础阶段」「高数」") String title,
            @ToolParam(description = "事件日期，格式 yyyy-MM-dd。传空字符串则删除所有匹配标题的事件") String date,
            ToolContext toolContext) {
     ...
     * 仅清空今日事件（不影响其他日期）
     */
    @Tool(description = "仅清空用户今日的所有日历事件。用于「清空今天的日程」「把今天的任务全删掉」「取消今天所有安排」等")
    public String clearToday(ToolContext toolContext) {
        Long userId = ToolContextHolder.userId(toolContext);
        if (userId == null) return "无法获取用户信息";
     ...
     * 清空用户所有日历事件（核弹操作，慎用）
     */
    @Tool(description = "清空用户所有日历事件（全部历史数据，不可恢复）。仅当用户明确说「清空所有日历」「全部删掉」「把整个日历清空」等才调用此方法。" +
            "如果用户说的是「清空今天」「删除今天」「取消今天」等仅限今日的操作，请调用 clearToday 而不是此方法")
    public String clearAll(ToolContext toolContext) {
        Long userId = ToolContextHolder.userId(toolContext);
        if (userId == null) return "无法获取用户信息";
